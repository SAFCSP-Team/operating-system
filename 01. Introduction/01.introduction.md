...
When clicking a program icon to run it, the operating system loads the program from disk to the main memory with a specific structure. The program then is called a process. A `process` is a program in execution. 
...
Every thing the operating system revolves around processes. It is really important to understand processes to conclude the over all picture of how operating system works.
