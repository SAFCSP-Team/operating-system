...
Lets have an example of creating a simple C program printing `Hello World` and try to modify it to use process syscalls. 
...
The fork returns a number, which is the process id, we can store it in an integer variable using `int` datatype, or we can use `sys/types` and use `pid_t` as the type of the process id.
