...
You do the following on the pointers 
...
This can helps us to access other data in the program with out referencing them directly, lts have an example. 
...
<!-- since they are local variables, they will take spaces from higher addresses to lower.  -->
...
We conclude that in order to get the next element, we will offset by the size of the data type in the array.
...
To enhance the CPU performance, padding by 3 byte will be added to align data properly. 

![int stored in memory with padding ](./images/11.int-with-padding.png)

By padding, the int value will be stored at address `0x0000C` which needs one fetch cycle to be read and write.
