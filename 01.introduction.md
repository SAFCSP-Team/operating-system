...
The previous image represents the actual device. But, if we look differentially at the concept, we will find that the logical representation of the computer system is just as hardware that is missing a piece which is the software to be a complete computer system that runs and performs an action. 


![image 2](./images/02.logical-view-of-computer.png)

General computer systems can hold several different programs and run them. You don’t have to mount and unmount the software manually. This is achieved using an operating system. 
...
If A is also done by the writer as well, he will line up after C to use the printer.
...
When an operating system is used, it will be a mediator between the hardware and the software, Therefore, its representation is going to be as the following. 
